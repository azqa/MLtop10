...
def get_abstracts():
    ...
    total_actual = 0
    for year in YEARS:
        year_url = urljoin(ROOT, 'search?q=ndss%20year%3A{0}%3A'.format(year))
        ...
        for head in heads:
            for link in head.find_all('a'):
                ...
                #print(link)
                # Only abstracts
                if paper_url and 'https://www.ndss-symposium.org/' in paper_url: # find the doi url
                    ...
                    paragraphs = abstract_soup.find('article', {'class':'ndss-paper'}).findAll('p')#, {'id': 'abstract'})
                    #print(paragraphs)
                    text = ''
